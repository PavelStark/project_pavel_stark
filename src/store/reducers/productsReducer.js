  ...
  switch (action.type) {
    ...
    case SORT:
      ...
      if (action.payload === "price_low_hight") {
        ...
        let sorted = state.products.sort((a, b) => a.percent - b.percent);
        return { products: sorted };
        //--------------sale_hight_low
      } else if (action.payload === "sale_hight_low") {
        let sorted = state.products.sort((a, b) => b.percent - a.percent);
        return { products: sorted };
        //--------------title_A_Z
        ...
        let sorted = state.products.sort(
          (a, b) => new Date(b.updatedAt) - new Date(a.updatedAt)
        );
        return { products: sorted };
        //--------------default
      } else if (action.payload === "default") {
        let sorted = state.products.sort((a, b) => a.id - b.id);
        return { products: sorted };
        //-------------------------------return-state-------
      } else {
        return state;
      ...
      if (action.payload) {
        return {
          ...
          products: state.products.map((elem) => {
            if (
              !(
                elem.truePrice >= action.payload.from &&
                elem.truePrice <= action.payload.to
              )
            ) {
...
export const allSalesProductsAction = (payload) => ({
  type: ALL_SALE_PRODUCTS,
  payload,
});
export const topSalesProductsAction = (payload) => ({
  type: TOP_SALES_PRODUCTS,
  payload,
});
